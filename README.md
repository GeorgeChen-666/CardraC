...
你可以点击“多图填充卡背”，选择多张图片设定卡背。

**添加文件-进阶说明**

我们也可以在添加对话框中一次同时指定卡面和卡背。

具体操作：在选择完文件卡面后先不要点击确定按钮。点击锁定正面按钮将选择的卡面图锁定，然后再选择卡背图，全部选择完毕后再点击确定按钮。
...
当我们设定成型后，可以点击设定弹窗的保存按钮，给当前配置存成模板，并在下拉菜单中点击模板名灵活切换不同模板。
...
选择一张图片，将选中的文件正面都设置成此图。

**卡面->多图填充卡面**

选择多张图片，将选中的文件正面逐个依序设置成选中的图片（如图片不足则后续卡牌不改变卡面，如果选择图片过多则忽略多选的图片）。
...
与文件设定菜单中的独立设置相同，只不过这里是批量设定所有选中的文件。

## 6 打印

点击工具栏中的打印按钮，侧边会弹出一个打印配置菜单。
...
居中偏移 X/Y：设定页面的打印偏移，为了修正双面打印的正反对齐。与项目设置页面的横纵偏移不同的是：这里如果设置向左偏移，正面背面全部向左偏移（正反会错位），项目设置中背面会向右偏移（正反同步偏移）。如果不会设置居中偏移，可以使用向导。
...
系统会自动根据四个结果计算出应该设定的偏移量，点击应用结果按钮接受设定。
...
图标的文件夹遵循下面的规范，这里使用的是语言缩写而不是国家缩写。
https://github.com/AnandChowdhary/language-icons

所支持的配置文件名如下：
https://app.unpkg.com/language-icons@0.3.0/files/icons
