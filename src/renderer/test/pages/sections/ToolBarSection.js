...
export class ToolBarSection extends BasePage {
  constructor() {
    super();
  ...
  async clickMenuItem(label) {
    await this.user.click(this.getMenuItem(label));
  }

  async toggleCheckbox(label) {
    await this.user.click(this.getCheckbox(label));
  }
  ...
  assertMatches(expectation) {
    ...
    if (expectation.visibleTextboxesCount !== undefined) {
      expect(screen.queryAllByRole('textbox')).toHaveLength(expectation.visibleTextboxesCount);
    }

    if (expectation.hiddenTextboxesCount !== undefined) {
      expect(screen.queryAllByRole('textbox')).toHaveLength(expectation.hiddenTextboxesCount);
    }
  }
}
